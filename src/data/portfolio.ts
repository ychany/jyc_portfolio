...
export const profileData = {
  ...
  devlinks: "", // 숨김 (기존: https://jyc-devlinks.vercel.app)
...
export const projectsData: {
  ...
  },
  {
    ...
    techStack: ["Next.js", "TypeScript", "TanStack Query", "Zustand", "Vapor UI"],
    ...
    notes: [
      "Next.js App Router 기반 페이지 구조 설계 및 라우팅 구현",
      "TanStack Query로 서버 상태 캐싱·재조회 관리, Zustand로 즐겨찾기·사용자 정보 등 전역 상태 분리",
      "디자이너의 Figma 컴포넌트와 Vapor UI를 연동해 반응형 디자인 시스템 구축 및 개발 생산성 향상",
      "Open API Generator로 Swagger 기반 API 호출 로직을 자동 생성해 백엔드 연동 비용 절감",
      "SSG로 숙소 상세 페이지를 빌드 타임 프리렌더링하고, Metadata API로 링크 공유 미리보기 제공",
      "LocalStorage를 활용해 스플래시 페이지 반복 노출 방지 등 사용자 경험 디테일 개선",
      "환경변수 스위치 기반 Mock API Layer를 구성해 백엔드 종속성 없이 프론트엔드 단독 배포 가능하도록 설계",
      "짧은 해커톤 기간 동안 팀 단위로 기획·디자인·개발을 동시에 진행하며 협업 프로세스 경험",
    ],
    pdf: {
      label: "발표자료 다운로드",
  ...
  // 숨김 처리 (프로젝트 목록에서 미노출) — Portfolio & DevLinks
  // {
  //   id: 7,
  ...
  },
  {
    ...
    techStack: ["Flutter", "Dart", "Riverpod", "GoRouter", "Dio", "Naver Map API", "Firebase", "Swift", "ActivityKit", "WidgetKit"],
    ...
    features: [
      ...
      "다이나믹 아일랜드·잠금화면 카드·홈/잠금화면 위젯으로 여행 D-day 표시",
      "앱 업데이트 안내",
    ],
    notes: [
      "App Store 정식 출시 (2026.08) 후 지속 업데이트 — 기획부터 배포·운영까지 프론트엔드 전 과정 담당",
      ...
      "Swift 네이티브로 다이나믹 아일랜드·잠금화면 카드(ActivityKit)와 홈·잠금화면 위젯(WidgetKit)을 직접 구현",
